...
export function ResultatsParacliniquesTab({ patientId }: ResultatsParacliniquesTabProps) {
  ...
  const handleMarkLu = async (resultatId: string) => {
    try {
      await apiJson(`/api/dossier-patient/${encodeURIComponent(patientId)}/resultats/${encodeURIComponent(resultatId)}/lu`, {
        method: "PATCH",
      });
      load();
    } catch (e) {
  ...
  if (isLoading) {
    return <p className="text-xs text-on-surface-variant">Chargement des résultats…</p>;
  }

  if (!resultats || resultats.length === 0) {
    return (
      <p className="text-xs text-on-surface-variant">
        Aucun résultat paraclinique disponible pour l&apos;instant pour ce patient.
      </p>
    );
  }

  const types = Array.from(new Set(resultats.map((r) => (r.type || "").toLowerCase()).filter(Boolean)));
  const filtered = filterType ? resultats.filter((r) => (r.type || "").toLowerCase() === filterType) : resultats;
